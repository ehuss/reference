...
impl Grammar {
    fn find_missing_nt(&self, diag: &mut Diagnostics) {
...
pub fn insert_grammar(grammar: &Grammar, chapter: &Chapter, diag: &mut Diagnostics) -> String {
    // Create a map of production name -> relative link path.
    let current_path = chapter.path.as_ref().unwrap().parent().unwrap();
    let link_map: HashMap<_, _> = grammar
        .productions
        .values()
        .map(|p| {
    ...
    let mut content = GRAMMAR_RE
        .replace_all(&chapter.content, |cap: &Captures<'_>| {
            ...
            output.push_str(
                "<div class=\"grammar-content grammar-text\">\n\
                 \n",
            );
            ...
            output.push_str(
                ...
                <div class=\"grammar-content grammar-railroad grammar-hidden\">\n\
                \n",
            );

            if let Err(e) = grammar.to_railroad(&names, &link_map, &mut output) {
                warn_or_err!(
                    diag,
                    "grammar failed in chapter {:?}: {e}",
                    chapter.source_path.as_ref().unwrap()
    ...
    for (name, path) in &link_map {
        writeln!(content, "[{name}]: {path}#grammar-{name}").unwrap();
    }
    content
}
