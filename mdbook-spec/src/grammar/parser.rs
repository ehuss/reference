...
pub fn parse_grammar(
    ...
    loop {
        let p = parser.parse_production(path)?;
...
impl Parser<'_> {
    ...
    fn parse_production(&mut self, path: &Path) -> Result<Production> {
        let name = self.parse_name().ok_or_else(|| self.error("expected production name".to_string()))?;
        self.space0();
    ...
    fn parse_expr1(&mut self) -> Result<Option<Expression>> {
        ...
        let mut kind = if self.take_str("U+") {
            ...
            .unwrap_or(false)
        {
            self.parse_nonterminal().expect("first char already checked")
        ...
        Ok(Some(Expression { kind, suffix, footnote }))
    }
    ...
    fn parse_footnote(&mut self) -> Result<Option<String>> {
        static FOOTNOTE_RE: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"^([^\]\n]+)]").unwrap());
        if !self.take_str("[^") {
            return Ok(None);
        }
...
fn box_kind(kind: ExpressionKind) -> Box<Expression> {
    Box::new(Expression { kind, suffix: None, footnote: None })
}
