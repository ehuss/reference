...
pub fn parse_grammar(input: &str, grammar: &mut Grammar, is_lexer: bool) -> Result<()> {
...
impl Parser<'_> {
    ...
    fn parse_expr1(&mut self) -> Result<Option<Expression>> {
        ...
        if let Some(cap) = self.take_re(&REPEAT_RE) {
            ...
            kind = ExpressionKind::RepeatN(box_kind(kind), a, b);
        }

        let suffix = self.parse_suffix()?;

        Ok(Some(Expression { kind, suffix }))
    }

    fn parse_nonterminal(&mut self) -> Result<ExpressionKind> {
        let nt = self.parse_name()?;
        Ok(ExpressionKind::Nt(nt))
    }
    ...
    fn parse_charset(&mut self) -> Result<ExpressionKind> {
        ...
        loop {
            self.space0();
            let Ok(ch) = self.parse_characters() else {
                break;
            };
            characters.push(ch);
        }
    ...
    fn parse_characters(&mut self) -> Result<Characters> {
        ...
        if let Some(cap) = self.take_re(&RANGE_RE) {
            ...
            Ok(Characters::Range(a, b))
        } else if let Some(cap) = self.take_re(&TERMINAL_RE) {
            Ok(Characters::Terminal(cap[1].to_string()))
        } else {
            let name = self.parse_name()?;
            Ok(Characters::Named(name))
        }
    }
    ...
    fn parse_neg_expression(&mut self) -> Result<ExpressionKind> {
        ...
        let kind = match next {
            ...
            _ => self.parse_nonterminal().map_err(|_| {
    ...
    fn parse_suffix(&mut self) -> Result<Option<String>> {
        ...
        loop {
            ...
            match next {
                ...
                b'_' if !in_backtick => {
                    if self.peek().map(|b| matches!(b, b'\n' | b' ')).unwrap_or(true) {
                        break;
                    }
                }
                _ => {}
...
fn box_kind(kind: ExpressionKind) -> Box<Expression> {
    Box::new(Expression { kind, suffix: None })
}
