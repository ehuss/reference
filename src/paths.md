...
r[paths.expr.intro]
Paths in expressions are used to refer to local variables and items, and they are also used in [patterns][patterns.path]. Optional [generic arguments] can be specified to supply concrete types, lifetimes, and constants for generic parameters.
